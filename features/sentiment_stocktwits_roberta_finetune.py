"""
Phase 2 extension: fine-tune the StockTwits RoBERTa model on our own labeled
StockTwits data, instead of running it zero-shot as
sentiment_stocktwits_roberta.py does. Reuses sentiment_tfidf.py's exact data
loading and train/test split (same TEST_SIZE/RANDOM_STATE/stratify) so
held-out accuracy is directly comparable to TF-IDF/LogReg, FinBERT
(zero-shot and fine-tuned), and this model's own zero-shot result — all on
identical data.

Unlike the FinBERT fine-tune, no head resize/relabeling trick is needed
here: the model's own label set is already binary (Negative/Positive,
confirmed via AutoConfig.id2label in sentiment_stocktwits_roberta.py), which
matches our bullish/bearish ground truth exactly.
...
from features.sentiment_stocktwits_roberta import score_batch, MODEL_NAME, BATCH_SIZE

METHOD = "stocktwits_roberta_finetuned"
EPOCHS = 3
LR = 2e-5
...
def run(tickers: list[str] = None) -> None:
    ...
    ft.evaluate_binary(preds, y_test, "StockTwits-RoBERTa (fine-tuned)")
    ...
    conn.close()
    log.info("StockTwits-RoBERTa fine-tuning done.")
